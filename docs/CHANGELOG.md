...
- แก้การ์ดสรุปด่วนบนสุดของ `/reports` (`ReportsPage.tsx`) ไม่ผูกกับช่วงที่กำลังเลื่อนดู: เจอระหว่าง UAT จริงของวี — เดิม (v1.0) `SummaryCard` ตรึงไว้ที่เดือนปฏิทินจริงเสมอ (`getRange(new Date(), 'month')`) ไม่ผูกกับ `anchor`/`periodType` ของแท็บด้านล่างเลย เลื่อนดูเดือนอื่นแล้วตัวเลขบนการ์ดไม่ขยับตาม สับสนว่าทำไมไม่เปลี่ยน
  ...
  - ตัดสินใจร่วมกับวี: เดิมเสนอแยกพฤติกรรม (net worth ตรึงปัจจุบัน, กระแสเงินสด/กำไรตามช่วงที่เลือก) แต่วีเลือกให้เปลี่ยนทั้ง 3 การ์ดตามช่วงที่เลื่อนดูเหมือนกันหมด เพราะรายงานด้านล่างก็เปลี่ยนอยู่แล้ว ทำให้ตัวเลขบนการ์ดตรงกับที่กำลังดูอยู่เสมอ
...
- แก้บั๊กงบกระแสเงินสด (`buildCashFlowStatement`, `src/features/reports/reportCalculations.ts`): เจอระหว่าง UAT จริงของวี (ทดสอบใช้งานต่อเนื่องหลัง C7) — สองเคสที่เงินสดไหลออกจริงแต่ไม่ถูกนับในกระแสเงินสดสุทธิเลย:
  ...
  - พบจากข้อมูลจริงของวี: โอนชำระ KTC 5,000 บาท + โอนให้ยืมเงิน Tanut (ลูกหนี้) 4,000 บาท ในเดือนเดียวกัน หายไปจากกระแสเงินสดสุทธิรวม 9,000 บาท ทำให้กระทบยอดกับเงินสดจริงไม่ตรง (77,314 → 67,759.61 หลังแก้ ตรวจสอบตรงกับ DB จริงแล้ว)
...
- ไม่มี migration ไม่กระทบ schema
- ทดสอบผ่าน: (1) จำลองการคำนวณด้วย DB จริงของวี (query legs เดือน ก.ค. 69 ตรงๆ) ยืนยันตัวเลขใหม่ตรงตามคาด (2) live browser testing ด้วย test user ชั่วคราว (รายรับ 50,000 − โอนชำระบัตร 5,000 − โอนให้ยืมเงิน 4,000 = กระแสเงินสดสุทธิ 41,000 ตรงบนหน้าจอจริง) — `tsc --noEmit`/`oxlint` ผ่านสะอาด
...
- **M7: Projection engine** — แท็บ "คาดการณ์" ใหม่ใน `/budget` คำนวณกระแสเงินสด + net worth ล่วงหน้าจาก `budget_items` เท่านั้น (ไม่รวม `recurring_transactions` จริงจาก M4) ผูก tax engine จริง (C6) ต่อปี พร้อม extrapolate config เกินปีที่มี (2568/2569) — ผ่าน golden case ตาม REQUIREMENTS §5.3 ตรงทุกจุด (ปิดบัญชี ก.ค.69, N=5, M=2 → ปีที่ 1 รายเดือน 5 เดือน, ปีที่ 2 รายเดือนเต็มปี, ปีที่ 3-5 รายปี)
  ...
  - ค่าลดหย่อนที่ใช้คำนวณภาษีในทุกปีคาดการณ์ = เฉพาะรายการที่วีติ๊ก **"ใช้ในงบประมาณ"** ในหน้า `/tax` แท็บค่าลดหย่อน (checkbox ใหม่ต่อรายการ, migration เพิ่ม `use_in_projection`) — ตกลงเพิ่มระหว่างแชตแทนการดึงค่าลดหย่อนปีล่าสุดทั้งหมดมาอัตโนมัติ เพราะบางรายการเป็นยอดเฉพาะปีนั้นจริงๆ
- **⚠️ เปลี่ยนสถาปัตยกรรมกลางแชต: รวมชั้น A (`budget_baseline_items`) + ชั้น B (`budget_schedule_items`) เป็นตารางเดียว `budget_items`** — วิเคราะห์ร่วมกับวีแล้วพบว่าชั้น B เป็น superset ของชั้น A ทุกกรณี การแยกสร้างความกำกวมจริง (ความหมาย "ต่อปี" ต่างกันคนละแบบ, บั๊กเลือกหมวด) โดยไม่ได้ลดความซับซ้อนของฟอร์มลงจริง — migrate ข้อมูลจริงที่วีกรอกไว้แล้ว (7+5 รายการ) ครบไม่มีข้อมูลหาย ดูรายละเอียดเหตุผล/schema ใหม่ใน `SPEC-budget.md`
