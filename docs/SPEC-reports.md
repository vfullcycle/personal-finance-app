...
`/reports` (nav ใหม่ "📊 รายงาน") — การ์ดสรุปด่วนบนสุด (Net worth / เงินสดคงเหลือ / กระแสเงินสดสุทธิ / กำไรสุทธิ **ตามช่วงที่กำลังเลื่อนดูอยู่** — v1.3/v1.4, ดูหมายเหตุด้านล่าง) + แท็บ 4 อัน ตาม `.tabs`/`.card`/`.item-row` component เดิมของแอป Mobile: แถวรายงาน stack (ชื่อ+ยอด+% เปลี่ยนแปลง), Desktop (`≥620px`): แถวเดียวคอลัมน์เดียวกัน — ไม่ต้องทำ horizontal-scroll เพราะแถวไม่กว้างมาก การ์ดสรุป: มือถือ grid 2x2, desktop (`≥620px`) แถวเดียว 4 คอลัมน์
...
**v1.4 เพิ่มการ์ด "เงินสดคงเหลือ":** วีขอมาแต่แรกเป็น "กระแสเงินสดสุทธิสะสมในปี" เพื่อตอบคำถาม "ตอนนี้เหลือเงินสดเท่าไหร่" แต่คุยกันแล้วพบว่ากระแสเงินสดสุทธิ (ต่อให้สะสมทั้งปี) บอกแค่**การเปลี่ยนแปลง**ของเงินสดในช่วงนั้น ไม่ใช่ยอดคงเหลือจริง — ยังต้องรู้ยอดตั้งต้นมาบวกเองอยู่ดี (นี่คือสาเหตุที่วีต้องเอายอดเดือนปัจจุบันไปหักลบเดือนก่อนหน้าเองที่ผ่านมา) เปลี่ยนแนวทางเป็นผลรวมยอดบัญชี `asset_liquidity=liquid` ณ `range.asOf` (สูตรเดียวกับ `liquidAssets` ที่ใช้คำนวณ Liquidity Ratio ใน C5 `ratioCalculations.ts`) ตอบคำถามตรงๆ ไม่ต้องคำนวณสะสมเอง ถ้าต้องการมุมมองรายปีเลื่อนตัวกรองด้านล่างเป็น periodType=year ได้เลยแทนที่จะมีการ์ดแยก (วีเห็นตรงกันว่าไม่ต้องมีการ์ดที่ 5 สำหรับกรณีนี้)
